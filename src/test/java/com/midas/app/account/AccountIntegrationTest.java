...
public class AccountIntegrationTest extends AbstractTest {
  ...
  void setUp() {
    ...
    when(stripePaymentProvider.createCustomer(any(Account.class)))
        .thenReturn("default-provider-customer-id");
  }
  ...
  @Transactional
  @DisplayName(
      "Given createAccountDto, When the provider in unavailable, Then return internal error")
  void testFailureCreateUserAccountForPaymentProvider() throws Exception {
    var createAccountDto =
        new CreateAccountDto().firstName("Ramon").lastName("Sharif").email("ramon@gmail.com");
    ...
    mvc.perform(request).andDo(print()).andExpectAll(status().isInternalServerError());

    Assert.assertEquals(0, accountRepository.count()); // TODO: rollback
  }
